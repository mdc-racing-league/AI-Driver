...
# Parallel racing-line profile per segment:
# list of (start_m, end_m, entry_pos, apex_pos, exit_pos). All positions are
# trackPos targets in [-1, 1]; linearly interpolated entry->apex at mid, then
# apex->exit. Defaults to 0 (centerline) if not set in YAML.
LINE_PROFILE: list[tuple[float, float, float, float, float]] = []
...
def target_track_pos_for(dist: float) -> float:
    """Return target trackPos in [-1, 1] for the given distance from start.

    Interpolates entry->apex->exit linearly across the segment: t in [0, 0.5]
    ramps entry->apex; t in [0.5, 1] ramps apex->exit. Returns 0.0 (centerline)
    outside any profile segment.
    """
    if not LINE_PROFILE:
        return 0.0
    last = LINE_PROFILE[-1]
    for start, end, entry, apex, exit_ in LINE_PROFILE:
        hit = (start <= dist < end) or (
            (start, end, entry, apex, exit_) == last and dist == end
        )
        if hit:
            length = end - start
            if length <= 0:
            ...
            t = (dist - start) / length
            if t <= 0.5:
                return entry + (apex - entry) * (t * 2.0)
            return apex + (exit_ - apex) * ((t - 0.5) * 2.0)
    return 0.0
...
def drive(state: dict) -> dict:
    ...
    dist = float(state.get("distFromStart", state.get("distRaced", 0.0)) or 0.0)
    target_pos = target_track_pos_for(dist)

    if abs(track_pos) > 1.0:
        steer = -OFFTRACK_RECOVERY_STEER if track_pos > 0 else OFFTRACK_RECOVERY_STEER
    else:
        steer = (angle - (track_pos - target_pos) * TRACK_POS_BIAS) / STEER_LOCK
...
_SEGMENT_SCALAR_KEYS = (
    "start_m", "end_m", "target_speed_kmh",
    "entry_pos", "apex_pos", "exit_pos",
)
...
def _load_segments(path: Path) -> list[dict]:
    ...
    scalar children. Optional `entry_pos`, `apex_pos`, `exit_pos` drive the
    racing-line profile (default 0 = centerline). Anything else is ignored.
    """
    text = path.read_text()
    segments: list[dict] = []
    in_segments = False
    ...
    def _flush():
        if {"start_m", "end_m", "target_speed_kmh"} <= current.keys():
            segments.append({
                ...
                "end_m": current["end_m"],
                "target_speed_kmh": current["target_speed_kmh"],
                "entry_pos": current.get("entry_pos", 0.0),
                "apex_pos": current.get("apex_pos", 0.0),
                "exit_pos": current.get("exit_pos", 0.0),
            })
    ...
    for raw_line in text.splitlines():
        ...
        key = key.strip()
        value = value.strip()
        if key in _SEGMENT_SCALAR_KEYS and value:
            try:
...
def main(argv: list[str] | None = None) -> int:
    ...
    global TARGET_SPEED_KMH, SLOW_ZONES, SEGMENTS, LINE_PROFILE, LOOKAHEAD_M, LOOKAHEAD_DECEL_MS2, BRAKE_TEST_SPEED, FULL_PEDAL_BRAKE, MAX_STEPS
    TARGET_SPEED_KMH = args.target_speed
    SLOW_ZONES = _parse_slow_zones(args.slow_zone)
    _raw_segments = _load_segments(args.segments) if args.segments else []
    SEGMENTS = [(s["start_m"], s["end_m"], s["target_speed_kmh"]) for s in _raw_segments]
    LINE_PROFILE = [
        (s["start_m"], s["end_m"], s["entry_pos"], s["apex_pos"], s["exit_pos"])
    ...
    if SEGMENTS:
        line_count = sum(
            1 for _, _, e, a, x in LINE_PROFILE if (e != 0.0 or a != 0.0 or x != 0.0)
        )
        print(
            f"[driver_baseline] segments: {len(SEGMENTS)} from {args.segments} "
            f"(racing-line nonzero on {line_count} segs)"
        )
    for start, end, speed in SLOW_ZONES:
